...
async function openOverlayPopout() {
  ...
  const canDocPip = Boolean(window.documentPictureInPicture?.requestWindow);
  if (canDocPip) {
    try {
      // IMPORTANT: If this fails on a hosted site, falling back to a normal popup
      // will NOT stay on top. So we prefer to surface the failure clearly.
      ...
      alert(
        "Document Picture-in-Picture failed to open.\n\n" +
          "Your browser supports it, but it was blocked or failed on this site.\n" +
          "Check browser permissions/policies (or extensions) and try again.\n\n" +
    ...
    win = window.open("", "sfOverlayPopout", `width=${width},height=${height}`);
    if (!win) {
...
function copyStylesToDoc(targetDoc) {
  ...
  // Ensure relative URLs (css hrefs, image urls inside inline styles, etc.)
  // resolve against the opener document, not about:blank.
  const baseHref = document.baseURI || window.location.href;
  if (baseHref) {
    let baseEl = head.querySelector("base");
    if (!baseEl) {
      baseEl = targetDoc.createElement("base");
      head.prepend(baseEl);
    }
  ...
  document.querySelectorAll('link[rel="stylesheet"]').forEach((node) => {
    ...
    if (href) {
      try {
        clone.setAttribute("href", new URL(href, baseHref).toString());
      } catch {
        // keep original href if URL() fails
      }
    }
